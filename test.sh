...
# Required: sudo apt install strace
...
main() {
    ...
    for i in $(seq 1 99); do
        ...
        # Copy file for freverse
        cp "$input_file" "$input_for_freverse"
        
        # Reverse file in Python (reference)
        reverse_file_python "$input_file" "$output_py"
        
        # Run freverse with strace
...
# Check if freverse exists
if [ ! -f "./freverse" ]; then
    echo "❌ ERROR: ./freverse was not found"
    exit 1
fi
